...
sub rmRDT{
my $usage="eupanSLURM rmRedundant [commands]

eupanSLURM rmRedundant is used to cluster sequences and extract the representative ones.
...
my $usage="\nUsage: eupanSLURM rmRedundant cdhitCluster [options] <input_fasta_file> <output_directory> <cdhit_directory>

eupanSLURM rmRedundant cdhitCluster is used to cluster contigs and remove the redundant ones.
...
die("Executable adjCdhit cannot be found in your PATH!\n
") unless($fpflag);
...
#************** Might be modified for different task submission system *******************
    my $job_file=$tmp_dir."/job.slurm";   #script_file
    ...
    open(JOB,">$job_file")||die("Error: Unable to create job file: $job_file\n");
	    print JOB "\#!/bin/bash\n";
    print JOB "\#SBATCH --job-name=cdhitCluster\n";                          #job name
...
my $usage="\nUsage: eupanSLURM rmRedundant blastCluster [options] <input_fasta_file> <output_directory> <blast_directory>

eupanSLURM rmRedundant blastCluster is used to cluster contigs and remove the redundant ones.
...
die("Executable blastCluster cannot be found in your PATH!\n
") unless($fpflag);
...
#************** Might be modified for different task submission system *******************
    ...
    open(JOB,">$job_file")||die("Error: Unable to create job file: $job_file\n");
	    print JOB "\#!/bin/bash\n";
    print JOB "\#SBATCH --job-name=blastCluster\n";                          #job name
	    print JOB "\#SBATCH -p $opt_q\n" if defined $opt_q;   #queue name in the submission system
    print JOB "\#SBATCH --output=$out_file\n";                   #stdout
    print JOB "\#SBATCH --error=$err_file\n";                   #stderr
    print JOB "\#SBATCH -n $thread_num\n";                 #thread number
