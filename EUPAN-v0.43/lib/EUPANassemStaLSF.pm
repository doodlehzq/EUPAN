...
my $usage="\nUsage: eupanLSF assemSta [options]  <assembly_directory> <output_directory> <QUAST_directory> <reference.fa>

eupanLSF assemSta is used to map assembled contigs to reference and to check the statistics of assembled contigs (or scaffolds).
...
#************** Might be modified for different task submission system *******************
    my $job_file=$script_out."/".$s."_quast.lsf";   #script_file
    ...
    print JOB "\#BSUB -J $s","_quast\n";              #job name
#   print JOB "\#BSUB -q cpu\n";                     #queue name in the submission system
...
my $usage="\nUsage: eupanLSF mergeAssemSta <QUAST_output_directory_list> > output_statistics

eupan LSF mergeAssemSta is used to collect statistices info of assembly from quast.
